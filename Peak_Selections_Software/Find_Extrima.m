...
function [varargout] = Find_Extrima (Data, nMaxima, indStart)
...
if ~isempty(Plato_Left) && ~isempty(Plato_Right)
    ...
    % maxima in the middle of the plateau
...
% Checking wheather the minima and maxima vectors have more than 2
...
if numel(Maxima_Full) > 0
    if Maxima_Full(1) < Minima_Full(1)
        Minima_Full  = [round((Maxima_Full(1)+1)/2) Minima_Full];
        if length(Maxima_Full) == length(Minima_Full)
            Minima_Full  = [Minima_Full round((Maxima_Full(end)+length(Data))/2)];
        end;
    else
        if length(Maxima_Full) == length(Minima_Full)
            Minima_Full  = [Minima_Full round((Maxima_Full(end)+length(Data))/2)];
...
% chose.
if nargout > 0, varargout{1} = Maxima;                end;
if nargout > 1, varargout{2} = Minima_Left;           end;
if nargout > 2, varargout{3} = Minima_Right;          end;
