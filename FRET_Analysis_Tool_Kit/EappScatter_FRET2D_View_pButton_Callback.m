function EappScatter_FRET2D_View_pButton_Callback (hObject, eventdata)

handles        = guidata(hObject);
[handles, ~]   = Calculate_Eapp_Conc_2D_Hist(handles, 'Scatter'); guidata(hObject, handles);
guidata(hObject, handles);

hFigure        = Plot_Window_Generic ('Eapp vs xA', hObject);
...
if length(handles.Conc_Val) > 2
    Plot_Eapp_vsXA_Conc(hObject);
else
    handles.xA_Bin = 0.05; guidata(hObject, handles);
    Plot_Conc_Dependent_Graph(hObject, 'Eapp vs xA')
end
