function FRET_2D_Calc_2DSpec_pButton_Callback(hObject, eventdata)

handles = guidata(hObject);
handles.Calc_2D_FRET_Switch = 1;

[handles, ~] = Calculate_Eapp_Conc_2D_Hist(handles, 'Surf');
guidata(hObject, handles);

if length(handles.Conc_Val) > 2
    % Norm_Factor = max(max(handles.Eapp_Hist_2D(1:end-1,:)));
    Hist3D     = handles.Eapp_Hist_2D;  %/Norm_Factor;
    ...
    % surf(handles.Conc_Val(1:end-1)+handles.Conc_BinSize/2, handles.Eapp_Val(1:end-1), 10*(log10(Hist3D+0.99)-log10(0.99)), 'EdgeColor','none', ...
    ...
    ylim([handles.Eapp_minRange handles.Eapp_maxRange]);
    ...
    %     'Position', [Conc_lBins(end)/2, (Eapp_lBins(1)-Eapp_lBins(2))*8.5/5, 0]);
    % set(yLbl,'String', ['E_a_p_p'], 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Rotation', -50, ...
    %     'Position', [(Conc_lBins(1)-Conc_lBins(2))*6/5, Eapp_lBins(end)/3, 0]);
    % set(zLbl,'String', 'Frequency', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    %     'Position', [(Conc_lBins(1)-Conc_lBins(2))*4/5, Eapp_lBins(end), ceil(max(Hist3D(:)))*cos(el/180*pi)]);
end

[handles, ~] = Calculate_Eapp_Conc_2D_Hist(handles, 'Stack');
guidata(hObject, handles);

Eapp_lBins    = [handles.Eapp_minRange:round((handles.Eapp_maxRange-handles.Eapp_minRange)/5,1):handles.Eapp_maxRange];
...
%     plot(handles.Eapp_Val, Hist3D, 'LineWidth', 1);
%     set(handles.hAxes, 'XTick',Eapp_lBins, 'FontSize',24);
end
