...
%------------------------------------------------------------------------------
function handles = FRET_1D_Calc_Hist_pButton_Callback(hObject, eventdata, FRET_FigureH)
...
if isfield(mwHandles, 'pcFigureH')
    if ~isempty(mwHandles.pcFigureH)
        ...
        handles   = handles.FRET_.Initiate(handles, mwHandles);
    end
else
...
if isempty(handles.FRET_Fit_Range)
    Fit_Range = [0, 1];
    handles.FRET_Fit_Range = Fit_Range;
    if handles.FRET_Hist_Bin >= 1, FRET_Hist_Bin = handles.FRET_Hist_Bin/100; else FRET_Hist_Bin = handles.FRET_Hist_Bin; end;
else
    if handles.FRET_Fit_Range <= 1
        if handles.FRET_Hist_Bin >= 1, FRET_Hist_Bin = handles.FRET_Hist_Bin/100; else FRET_Hist_Bin = handles.FRET_Hist_Bin; end;
        Fit_Range = handles.FRET_Fit_Range;
    else
        FRET_Hist_Bin = handles.FRET_Hist_Bin;
        Fit_Range     = handles.FRET_Fit_Range;
    end;
end;
...
for ii = 1:length(mwHandles.Current_Polygon_Index)
    ...
    if ~isempty(Segment_Found)
        FRET_Item = FRET_Item.Calculate_Histogram(Related_Segment.Polygons, FRET_Hist_Bin, Current_Image, Fit_Range);
    else
        FRET_Item = FRET_Item.Calculate_Histogram(Polygon.Coordinates, FRET_Hist_Bin, Current_Image, Fit_Range);
    end;
    ...
    % Add a FRET instance to the FRET list
