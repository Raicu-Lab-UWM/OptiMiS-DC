...
%------------------------------------------------------------------------------
function Self = Membrane_Select(Self, Image, Entity_Polygon, Det_Membrane_Thick)

if nargin < 3, Entity_Polygon = Self.Coordinates; elseif isempty(Entity_Polygon), Entity_Polygon = Self.Coordinates; end;

if isempty(Self.Mask), Mask = poly2mask(Entity_Polygon(:,1),Entity_Polygon(:,2),size(Image,1),size(Image,2));
else Mask = Self.Mask;
end;

if nargin < 4, MT = round(Self.Mem_Thick/2); elseif isempty(Det_Membrane_Thick), MT = round(Self.Mem_Thick/2); 
else MT = round(Det_Membrane_Thick/2); Self.Mem_Thick = MT;
end;
...
if length(Poly_List) == 2
    ...
    sprsMembrane_Polygon = Entity_Polygon;
end;

Self.Membrane       = sprsMembrane_Polygon;