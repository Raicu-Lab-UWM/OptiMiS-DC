...
% archive can be accessed by providing a '.zip' filename in
...
switch nTypeID
   case 1
      ...
      if (nShapeROISize > 0)
         ...
         n=1;
         m=1;
         while n<=(length(sROI.vfShapes)-3)
             if (sROI.vfShapes(n) > 4)
                 sROI.Coords(m,1) =sROI.vfShapes(n);
                 sROI.Coords(m,2) =sROI.vfShapes(n+1);
                 m=m+1;
                 n=n+2;
             else
                 n=n+1;
