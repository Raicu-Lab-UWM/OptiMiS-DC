function UnMix_pButton_Callback(hObject, eventdata)

handles = guidata(hObject);

UM_TK_FigureH   = figure('Name','UnMix Toolkit','NumberTitle','off', ...
                         'MenuBar', 'none', 'ToolBar', 'none', 'Units', 'Pixels', ...
                         'Position', [875, 200, 500, 615], 'Tag', 'UM_TK_FigureH');

umHandles           = guihandles(UM_TK_FigureH);
if nargin > 0, handles = guidata(hObject); umHandles.mwFigureH = hObject; umHandles.StandAlone = false; 
else, handles = []; umHandles.StandAlone = true;
end
umHandles.output    = UM_TK_FigureH;
...
umHandles.Scene_List_lBox = Scene_List_lBox;
...
umHandles = Initiate_Window_Param(umHandles, handles);
